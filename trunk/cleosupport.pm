...
sub load_user_conf( $;$ ) {
    ...
    # make corrections
    if($new_local_user_settings{$user}{priority}>get_setting('priority',$user)){
        $new_local_user_settings{$user}{priority} =
        get_setting( 'priority', $user );
    }
    if ($new_local_user_settings{$user}{def_priority}>get_setting('priority',$user)){
    ...
    if($new_local_user_settings{$user}{max_sum_np} >get_setting('max_sum_np',$user)){
        $new_local_user_settings{$user}{max_sum_np} =
        get_setting( 'max_sum_np', $user );
    }
    if($new_local_user_settings{$user}{max_cpuh} >get_setting('max_cpuh',$user)){
        $new_local_user_settings{$user}{max_cpuh} =
        get_setting( 'max_cpuh', $user );
    }
    if($new_local_user_settings{$user}{max_tasks}>get_setting('max_tasks',$user)){
        $new_local_user_settings{$user}{max_tasks} =
        get_setting( 'max_tasks', $user );
    }
    if($new_local_user_settings{$user}{max_time}>get_setting('max_time',$user)){
        $new_local_user_settings{$user}{max_time} =
        get_setting( 'max_time', $user );
    }
    if($new_local_user_settings{$user}{default_time}>get_setting('max_time',$user)){
        $new_local_user_settings{$user}{default_time} =
        get_setting( 'default_time', $user );
    }
    if($new_local_user_settings{$user}{max_tasks_on_pe}>get_setting('max_tasks_on_pe',$user)){
        $new_local_user_settings{$user}{max_tasks_on_pe} =
        get_setting( 'max_tasks_on_pe', $user );
    }
    if($new_local_user_settings{$user}{max_queue}>get_setting('max_queue',$user)){
        $new_local_user_settings{$user}{max_queue} =
        get_setting( 'max_queue', $user );
    }
    if($new_local_user_settings{$user}{exec_line}>get_setting('exec_line',$user)){
