...
#  (C) Sergey Zhumatiy (serg@parallel.ru) 1999-2011
#
#
...
BEGIN {
  $VERSION = 5.30;
  eval "use lib '.'; use lib '/usr/lib/cleo';use cleovars $VERSION";
  if ($@) {
    die "$@\n";
  }
  1;
...
@EXPORT = qw(
  ...
  &resurect_task &del_or_restart_task
);
...
%def_global_settings = (
  ...
  exec_line => '/opt/mpich/bin/mpirun -np $n -machinefile $file $wrap $task',
  exec_modules_dir     => '/usr/lib/cleo',
...
#'echo \'Node $node raises back very suspicious!\' |mail -s \'FAIL $node\' $adm_email',
  ...
  parent           => '',
  pbs_compat       => 1,                            # export PBS_* variables
  pe               => [],
  pe_sel_die_count => 10,
  pe_sel_method    => {},
  pe_select        => 'random',
  ...
  post_exec_text =>
    'Your task $sexe ($id) is finished with code $status. Worked: $time.',
  post_exec_write => '',
...
my %xml_no_print = (
  'rsh_filter', 1, 'start',     1, 'time',       1, 'added', 1,
  'own',        1, 'shared',    1, 'extranodes', 1, 'task',  1,
  'blocks',     1, 'timelimit', 1
);
...
# minutes, hours, week_day, month_day, month, 'flag'
sub disturb_if( \$,\$,\$,\$,\$,\$ ){
my \$p=__PACKAGE__;
cleosupport::set_mod_timer(\$p,\@_);
}
...
# launch program
sub launch (\$\$\$){
return cleosupport::launch(\$_[0],\$_[1],\$_[2]);
}
_MOD_PROLOG
...
sub launch( $$$ ) {
  ...
  if ($time) {
    ...
    sleep $time;
    exit(0)
      unless -f "/tmp/cleo-launch.$id";    # exit, if launch is not nessesary
...
sub count_user_np_used( ;$ ) {
  ...
  foreach $i (@running) {
    ...
    if (!defined $user or $i->{user} eq $user) {
      $user_np_used{$i->{user}} +=
        scalar(@{$i->{own}}) + scalar(@{$i->{shared}}) + $i->{npextra};
    }
  }
  foreach $i (@queue) {
...
#  qlog "Testing adm ($user) in (".join(';',@{$global_settings{admins}}).")\n", LOG_DEBUG;
  if (
    is_in_list($user, $global_settings{admins})
    || (
      $cluster
...
sub candebug($;$ ) {
  ...
  $cluster ||= get_setting('root_cluster_name');
  if (
    is_in_list($user, $global_settings{debug_users})
    || (
      $cluster
...
sub push_to_queue( $ ) {
  ...
  $entry->{timelimit} ||=
    get_setting('max_time', $entry->{user}, $entry->{profile}, $entry->{owner});
  ...
  if ($entry->{nice} eq '') {
    $entry->{nice} = get_setting('nice', $entry->{user}, $entry->{profile});
  }
  if ($entry->{gummy} eq '') {
...
#            $XML->print("   <cpu id=\"$2\" type=\"$t\" own=\"$w\" blocked=\"$b\">\n");
    ...
    #        $XML->print("</cpus>\n<tasks>\n <tasks_statistics>\n");
    $i =
      scalar(@running) + scalar(@foreign) + scalar(@pending) + scalar(@queue);
    ...
    $writer->dataElement('tasks-prerun',
      0 + grep($_->{state} eq 'prerun', @queue));
...
#                "</tasks-prerun>\n");
   ...
   #                "</tasks-blocked>\n");
    $writer->dataElement('tasks-queued',
      0 + grep($_->{state} eq 'queued', @queue));
...
#                      $writer->endTag('reason');
      ...
      #            $XML->print(" <nodes>\n");
      foreach $cpu (sort(@{$i->{own}}, @{$i->{shared}}, @{$i->{extranodes}})) {
        $cpu =~ /^(.*):(.*)$/;
        $writer->emptyTag(
          'item',
          'node_name' => $1,
...
"Cannot save queue status to '$queue_save.$name'. Try to use '$queue_alt_save.$name'\n";
    ...
    print "Cannot to create backup. Try to save to $queue_alt_save.$name\n";
    qlog "Cannot to create backup. Try to save to '$queue_alt_save.$name'\n";
    open(SAV, ">$queue_alt_save.$name")
      or die "Cannot save queue status to $queue_alt_save.$name.\n";
  }
  $block = '0';
    ...
    if (/^\#$/) {    #entry end mark
      if ($x->{id} eq 'global') {    # global settings
        ...
        eval {
          if (defined $x->{pe_blocks}) {
            foreach $k (keys(%{$x->{pe_blocks}})) {
              block_pe($k, 1, 0, keys(%{$x->{pe_blocks}->{$k}}))
        ...
        foreach $k (keys(%$x)) {
          if ($k eq 'rsh_data') {
            ...
            if (ref($x->{time_restrictions}) eq 'ARRAY') {
              @main::time_restrictions = @{$x->{time_restrictions}};
            } else {
        ...
        if ($x->{state} eq 'run') {
          my $t;
          push @running, $x;
          foreach $t (@{$x->{shared}}, @{$x->{own}}, @{$x->{extranodes}}) {
            $pe{$t}->{ids}->{$x->{id}} = $x->{pid};
          }
        ...
        if (exists $x->{oldid} and $x->{oldid} > 0) {
          $extern_ids{$x->{lastowner}}->{$x->{oldid}} = $x->{id};
        }
...
sub fix_prerun() {
  ...
  foreach $i (@queue) {
    ...
    my @nodes =
      map {/^([^:]*)/, $1} (@{$i->{shared}}, @{$i->{own}}, @{$i->{extranodes}});
    main::new_req_to_mon('cancel_attach', $i, \@nodes, SUCC_ALL | SUCC_OK,
      undef, undef);
  }
}
...
sub load_config( $;$$ ) {
  ...
  my $mode =
    1;   # 1-server, 2-users, 3-clusters, 4-profiles, 5-hybride, 6-groups, 7-mod
  my ($arg, $arg2, $q);
  my ($var, $val,  $tmp);
  ...
  while (<IN>) {
    ...
    if ($mode == 1) {    # global (or user-file)
      ...
      if ($user ne '') {
        if (
          $tmp = assign_new_value(\%{$new_local_user_settings{$user}},
            $var, $val, $unsafe, 'l') > 0
          ) {
        ...
        if ($tmp =
          assign_new_value(\%new_global_settings, $var, $val, $unsafe, 'g') > 0)
        {
      ...
      if (
        $tmp = assign_new_value(
          \%{$new_user_settings{$arg}}, $var, $val, $unsafe, 'u'
        ) > 0
        ) {
      ...
      if (
        $tmp = assign_new_value(\%{$new_cluster_settings{$arg}},
          $var, $val, $unsafe, 'q') > 0
        ) {
      ...
      if (
        $tmp = assign_new_value(\%{$new_profile_settings{$arg}},
          $var, $val, $unsafe, 'p') > 0
        ) {
      ...
      if (
        $tmp = assign_new_value(\%{$new_clusteruser_settings{$arg}{$arg2}},
          $var, $val, $unsafe, 'U') > 0
        ) {
...
sub get_report_filename(;$){
  ...
  if(defined $_[0]){
    ...
    $f=get_setting('log_file');
  }
  return "$f-$cluster_name";
}
		    
sub get_short_report_filename(;$){
  my $f;
  if(defined $_[0]){
    $f=$_[0]->{short_log_file};
  }
  else{
    $f=get_setting('short_log_file');
  }
  return "$f-$cluster_name";
}
...
sub post_load_config( $$ ) {
  ...
  if (exists($new_settings->{admins})
    ...
    @{$new_settings->{admins}} =
      &expand_groups_array(\%new_groups, \@{$new_settings->{admins}});
  } else {
  ...
  if (exists($new_settings->{users})
    ...
    @{$new_settings->{users}} =
      &expand_groups_array(\%new_groups, \@{$new_settings->{users}});
  } else {
  ...
  if (exists($new_settings->{nousers})
    ...
    @{$new_settings->{nousers}} =
      &expand_groups_array(\%new_groups, \@{$new_settings->{nousers}});
  } else {
  ...
  if (exists($new_settings->{queue_save})) {
    ...
    if (open(SAVE, ">$queue_save-$cluster_name")) {
      ...
      qlog "Cannot create new save file ($new_settings->{queue_save})\n";
    }
  }
  ...
  if (exists($new_settings->{log_file})) {
    my $old = $report_file;
    $opts{l} = $report_file = get_report_filename($new_settings);
    $STATUS->close();
  ...
  if (exists($new_settings->{short_log_file})) {
    ...
    $opts{L} = $short_rep_file = get_short_report_filename($new_settings);
    $SHORT_LOG->close();
    unless (
      $SHORT_LOG->open(
...
sub recreate_plugins_and_ports() {
  ...
  if ($is_master) {
    if (defined $main::LST) {
      ...
      $main::LST =
        new_listen Cleo::Conn(get_setting('port'),
        get_setting('listen_number'));
...
sub load_conf_file(;$ ) {    # safe by default!
  ...
  if (load_config($opts{c}, $unsafe) == 0) {
    ...
    foreach $i (keys(%pe_sel_method)) {
      qlog("Killing pe_sel_method '$i' (pid=$pe_sel_method{$i}->{pid})\n",
        LOG_INFO);
      kill_tree(9, $pe_sel_method{$i}->{pid});
    }
    undef %pe_sel_method;
    ...
    $max_mon_timeout =
      $new_global_settings{mon_ping_interval} +
...
sub post_load_conf_helper() {
  ...
  for $i (keys(%new_clusteruser_settings)) {
    for $j (keys(%{$new_clusteruser_settings{$i}})) {
      post_load_config($new_clusteruser_settings{$i}{$j}, \%new_groups);
    }
  }
}
...
sub create_cluster_structure( $ ) {
  ...
  qlog "my childs: " . join(',', @{$clusters{$cluster_name}->{childs}}) . "\n",
    LOG_INFO;
...
sub reload_users( ;$ ) {
  ...
  while (($user, undef, undef, $gid, undef, undef, undef, $dir, undef, undef) =
    getpwent()) {
    push @users, $user;
    $user_home{$user}   = $dir;
    $usergid{$user}     = $gid;
...
sub load_user_conf( $;$ ) {
  ...
  # make corrections
  if (
    $new_local_user_settings{$user}{priority} > get_setting('priority', $user))
  {
    $new_local_user_settings{$user}{priority} = get_setting('priority', $user);
  }
  ...
  if ($new_local_user_settings{$user}{max_np} > get_setting('max_np', $user)) {
    $new_local_user_settings{$user}{max_np} = get_setting('max_np', $user);
  }
  ...
  if (
    $new_local_user_settings{$user}{max_cpuh} > get_setting('max_cpuh', $user))
  {
    $new_local_user_settings{$user}{max_cpuh} = get_setting('max_cpuh', $user);
  }
  ...
  if (
    $new_local_user_settings{$user}{max_time} > get_setting('max_time', $user))
  {
    $new_local_user_settings{$user}{max_time} = get_setting('max_time', $user);
  }
...
sub deldir($;$ ) {    #delete directory recursively
  ...
  sub_exec(get_uid($u), $usergid{$u}, \&cleo_system, '/bin/rm', '-rf', "/$arg");
  return 0 if (-e "/$arg");
  return 1;
}    # deldir
...

{
  ...
  sub subst_task_prop( $$;$ ) {
    ...
    if (defined $child) {
      ...
      $subst_args{core}    = $func->($child->{core} ? 'core dumped' : '');
      ...
      $subst_args{mpi_nodes}        = $func->(cpus2uniq_nodes($child->{nodes}));
      ...
      $subst_args{exe}  = space_quote($func->($child->{task_args}->[0]));
      $subst_args{sexe} = $subst_args{exe};
      $subst_args{sexe} =~ s/^.*\/(.*?)/$1/;
      my @args = map(space_quote($func->($_)), @{$child->{task_args}});
      ...
      $subst_args{home}    = space_quote($func->($user_home{$child->{user}}));
      ...
      $map_file =
        get_setting('cpu_map_file', $child->{user}, $child->{profile});
      if ($map_file ne '') {
        my %map;
        ...
        #                }
        $mapped_nodes =
          join(',', map {$map{$_};} split(',', $child->{full_nodes}));
        $subst_args{mapped_nodes} = $func->($mapped_nodes);
        ($subst_args{mapped_node}) =
          map {$func->($map{$_});} "$child->{node}:$child->{nid}";
...
sub s_val($ ) {
  return $subst_args{$_[0]} if (exists($subst_args{$_[0]}));
  return '$' . $_[0];
}
...
sub make_subclusters($ ) {    #parameter - name of cluster
  ...
  $short_rep_file=get_short_report_filename();
  reopen_logs();
...
#
#  Creates config file for task just as one-per-line list
#
#  Args: filename - confog file template
#        entry    - task hash
...
sub create_config( $$$ ) {
  ...
  $t = get_setting('use_file_simple', $q_entry->{user}, $q_entry->{profile});
  if ($t ne '') {
    create_simple_config($t, $q_entry, $work_pe, $doit);
    return;
  }
  ...
  if ($t ne '') {
    ...
    if ($t = get_setting('file_line', $q_entry->{user}, $q_entry->{profile})) {
      my (
        %h,     @short_p, %u,        $a,     $b, $t2,
        $count, $max,     $collapse, $first, $f_line
      );
      for $a (@p) {
        ++$h{$a};
        ++$u{$a};
      ...
      qlog "Creating: @p\n", LOG_DEBUG;
      $collapse =
        get_setting('coll_nodes', $q_entry->{user}, $q_entry->{profile});
      $first =
        get_setting('use_first_line', $q_entry->{user}, $q_entry->{profile});
      $f_line =
        get_setting('first_line', $q_entry->{user}, $q_entry->{profile});
      foreach $a (@short_p) {
        $b               = $h{$a};
        $q_entry->{node} = $a;
        $q_entry->{n}    = $b;
...
######################################################
{
  ...
  sub run_via_mons( $ ) {
    ...
    qlog "User $ids{$id}->{user} Task ["
      . $ids{$id}->{task_args}->[0]
      . "] on $ids{$id}->{np} proc.\n", LOG_DEBUG;
    foreach my $pe (@{$ids{$id}->{shared}}, @{$ids{$id}->{own}},
      @{$ids{$id}->{extranodes}}) {
      $pe{$pe}->{ids}->{$ids{$id}->{id}} =
    ...
    my $t =
      get_setting('use_file_simple', $ids{$id}->{user}, $ids{$id}->{profile});
    if ($t eq '') {
    ...
    $childs_info{$id}->{nodes} =
      join(',', sort(@{$ids{$id}->{shared}}, @{$ids{$id}->{own}}));
    ...
    if ($cluster_name eq cleosupport::get_setting('root_cluster_name')) {
      ...
      $main::rsh_data{"$ids{$id}->{id}::$ids{$id}->{owner}"}->{"np_free"} =
        $ids{$id}->{np} - 1;
      $main::rsh_data{"$ids{$id}->{id}::$ids{$id}->{owner}"}->{"master"} =
        $work_pe[0];
      my %cpus;
      map {$cpus{$_} = 1} map {/^([^:]+)/; $1;} @work_pe;
      @{$main::rsh_data{"$ids{$id}->{id}::$ids{$id}->{owner}"}->{"nodesb"}} =
        keys(%cpus);    #"
      ...
      if (  ($ids{$id}->{use_rsh_filter} ne '')
        ...
        $ids{$id}->{group} = $groups;
        undef %main::rsh_cmd_lines;    #!!!! bugfix code. make it better...
        ...
        $main::rsh_pids{"$ids{$id}->{id}::$ids{$id}->{owner}"}->{master} =
          $work_pe[0];
      } else {
      ...
      main::answer_to_parent(cleosupport::get_setting('root_cluster_name'),
...
sub run_task( $ ) {
  ...
  qlog "Run_task: $id\n", LOG_INFO;
  qlog(
    "Try to exec [$q_entry->{task_args}->[0]], "
  ...
  if (get_setting('occupy_full_node', $q_entry->{user}, $q_entry->{profile})
    ...
    qlog "$q_entry->{npextra} extra cpus used ($extra_nodes_used totla))\n",
      LOG_INFO;
...
"Invalid request to run $id (from $q_entry->{owner}): state is $ids{$id}->{state}\n",
    ...
    for $t (@{$q_entry->{extranodes}}, @{$q_entry->{shared}},
      @{$q_entry->{own}}) {
      $pe{$t}->{ids}->{$id} = -1;
    }
    ...
    slog "RUN_NOT_OWN $id->{uniqid}; $q_entry->{user}; $q_entry->{owner}; "
...
"Was used by $user: $user_np_used{$user} ($q_entry->{np}+$q_entry->{npextra})\n",
  ...
  #  if (@{$q_entry->{shared}}>0) {
  {
    my %answ = (
      'id'    => $id,
      'nodes' => join(',', @{$q_entry->{shared}}, @{$q_entry->{extranodes}})
    );
    main::new_req_to_child(
      'run_pre',          \%answ,    '__all__',       0,
  ...
  if (0) {
    ...
    my @nodes =
      (@{$q_entry->{shared}}, @{$q_entry->{own}}, @{$q_entry->{extranodes}});
    if ($is_master) {
      main::new_req_to_mon(
        'do_pre',                   $req,
        \@nodes,                    SUCC_ALL | SUCC_OK,
        \&main::mon_do_pre_handler, undef,
        get_setting('mon_timeout'), \&main::mon_do_pre_handler,
        'owner',                    $q_entry->{owner},
        'id',                       $id,
      );
    } else {
  ...
  if ($q_entry->{run_via_mons}) {
    ...
    # Traditional run
    $q_entry->{nodes} =
      join(',', sort(@{$q_entry->{shared}}, @{$q_entry->{own}}));
    my %cpus;
    map {$cpus{$_} = 1}
      map {/^([^:]+)/; $1;} (@{$q_entry->{shared}}, @{$q_entry->{own}});
    ...
    if (get_setting('use_monitors') == 0) {
      ...
      cleosupport::execute_task($q_entry);
    }
    ...
    $t =
      ...
      $q_entry->{profile});
    if ($t ne '' and get_setting('use_monitors') > 0) {
      qlog "ATTACH ($t)\n", LOG_INFO;
      ...
      $t2 =
        cleosupport::get_setting('attach_parent_mask', $q_entry->{user},
        $q_entry->{profile});
      ...
      if ($is_master) {
        main::new_req_to_mon(
          ...
          $id,
          'tmout',
          get_setting('attach_timeout', $q_entry->{user}, $q_entry->{profile})
        );
        $ret = 0;
      } else {
        main::answer_to_parent(cleosupport::get_setting('root_cluster_name'),
...
#######################################################
#
#  Creates additional env. variables before task start
#
#  Args: q_entry - task description
#
sub create_task_env( $ ) {
  my $q_entry = $_[0];

  $ENV{CLEO_RUN_HOST} = $my_name;
  $ENV{CLEO_QUEUE}    = $q_entry->{queue};
  $ENV{CLEO_WORKDIR}  = $q_entry->{dir};
  $ENV{CLEO_JOBID}    = $q_entry->{id};
  $ENV{CLEO_NODEFILE} = $q_entry->{use_file};
  $ENV{CLEO_JOBNAME}  = $q_entry->{sexe};
...
sub execute_task( $ ) {
  ...
  $t = get_setting('use_exec_modules', $q_entry->{user}, $q_entry->{profile});
  if (defined($t)) {
    my $cancel = 0;
    foreach my $i (@$t) {
      ...
      if ($exec_mod_cancel eq 'restart') {
        ...
        $q_entry->{restart} = 1;
        del_or_restart_task($q_entry->{id},"RESTART by EXEC_MODULE ($i)");
...
#msgsnd($exec_queue, pack("l! a*",length($str),$str),1);
        ...
        #msgsnd($exec_queue, pack("l! a*",length($str),$str),1);
        $cancel = 1;
        del_task($q_entry->{id}, '__internal__', '', '', '', 1,
          'pre-start failed');
    ...
    if ($cancel == 1) {
      ...
      slog( "CANCEL: $q_entry->{uniqid}; $q_entry->{user}; "
          . "$q_entry->{np}; "
          . join(' ', @{$q_entry->{task_args}})
          . "\n");
      return -1;
  ...
  if ($q_entry->{attrs}->{mailopts} =~ /b/) {
    ...
    my $subject =
      cleosupport::get_setting('pre_exec_subj', $q_entry->{user},
      $q_entry->{profile});
    my $text =
      cleosupport::get_setting('pre_exec_text', $q_entry->{user},
      $q_entry->{profile});
  ...
  $pid = fork();
  unless (defined $pid) {
    qlog "execute_task failed ($q_entry->{task_args}->[0])" . " in fork\n",
      LOG_WARN;
  ...
  if ($pid) {    #parent
    ...
    if ($pid < 0) {
      ...
      qlog "Fork failed. User $q_entry->{user}"
        ...
        LOG_WARN;
      slog "RUN_FAIL $q_entry->{uniqid}; $q_entry->{user}; "
        . "$q_entry->{np}; "
        . join(' ', @{$q_entry->{task_args}}) . "\n";
      return -2;
    }
...
"TIMELIMIT: $childs_info{$id}->{timelimit} ($childs_info{$id}->{time})\n",
   ...
   #                 \&create_config, $q_entry, \@work_pe, 0 );
    ...
    #      local $,=';'; print "+> ",%pids,"\n";
    #    }
    slog "RUN $q_entry->{uniqid}; $q_entry->{user}; $q_entry->{np}; "
      . join(' ', @{$q_entry->{task_args}}) . "\n";
    slog "RUN_NODES $q_entry->{uniqid}; $q_entry->{user}; "
      . "$q_entry->{np}; $q_entry->{npextra}; $q_entry->{nodes}; "
      . join(',', @{$ids{$id}->{extranodes}}) . "\n";

    qlog "RUN $q_entry->{uniqid}; $q_entry->{user}; $q_entry->{np}; "
      . join(' ', @{$q_entry->{task_args}}) . "\n";
    qlog
      "$id/$ids{$id}->{state}/$childs_info{$id}->{state}/$q_entry->{state}\n";
    dump_queue();
    return $pid;
  } else {    #child
    ...
    # Create session via PAM
    my $pamh = new Authen::PAM('cleo-exec', $q_entry->{user});
    $pamh->pam_open_session();
    $pamh = 0;

    # Just ignore result - if session was not initiated we'll try to run
    # application anyway...
    ...
    $t = get_setting('q_pre_exec', $q_entry->{user}, $q_entry->{profile});
    if ($t ne '') {
      undef %subst_args;
      subst_task_prop(\$t, $q_entry);
      qlog "exec q_pre: '$t'\n", LOG_INFO;
    ...
    $t = get_setting('q_just_exec', $q_entry->{user}, $q_entry->{profile});
    if ($t ne '') {
      undef %subst_args;
      subst_task_prop(\$t, $q_entry);
    ...
    POSIX::nice(
      get_setting('task_local_renice', $q_entry->{user}, $q_entry->{profile}));
...
"Use uid=$<,$>; gid=$(,$) ($q_entry->{user}:$useruid{$q_entry->{user}}/$gid;$user_groups{$q_entry->{user}})\n",
   ...
   #                 \&create_config, $q_entry, \@work_pe, 1 );
    ...
    if ($t ne '') {
      ...
      my $u = $q_entry->{user};
      sub_exec(get_uid($u), $usergid{$u}, \&cleo_system, $t);
  ...
  if ($q) {
    ...
    if (@{$q->{shared}} > 0 or $q->{state} eq 'prerun') {
      ...
      main::new_req_to_child(
        'del', {'id' => $q->{id}},
        '__all__',                      0,
        SUCC_ALL | SUCC_OK,             \&main::chld_del_handler,
        \&main::chld_every_del_handler, get_setting('intra_timeout'),
...
sub count_free($$ ) {    # actually \@ and \%
  ...
  for $i (keys(%$pe)) {
    ...
    qlog("CF_PE=$i; count=" . scalar(keys(%{$p->{ids}})) . " max:$p->{max}\n",
      LOG_DEBUG)
      if ($debug{cf});
    push(@$free, $i)
      if (scalar(keys(%{$p->{ids}})) < $p->{max});
      ...
      if ($shared_too) {
        for $p (@{$clusters{$cluster_name}->{childs}}) {
          ...
          main::new_req_to_child(
            'del',                          \%args,
            '__all__',                      0,
            SUCC_OK | SUCC_ANY,             \&main::chld_del_handler,
            \&main::chld_every_del_handler, get_setting('intra_timeout'),
            \&main::chld_del_handler
          );
        }
...
sub autoblock( $$$ ) {
  ...
  $q_change = 1;
  return
      "+$cluster_name: autoblock for $userlist "
    . ($val ? "" : "re")
    . "was set";
}    #~autoblock
...
sub set_entry_priority( $$$$ ) {
  ...
  $user = uid2user($user);
  $max_pri =
    cleosupport::get_setting('priority', $user, $queue->[$i]->{profile});
  $e = $queue->[$i];
  if (!isadmin($user)) {
    if ($e->{user} ne $user) {
...
"----------------------------------------------------------------------\n";
  ...
  if (@foreign + @pending + @queue) {
    ...
    push @out, scalar(@foreign) . ";" . scalar(@pending) . ";" . scalar(@queue)
      if $tech;
    push @out, "\n";
...
"Free: $free_total\nShared_free: $free_shared_total\nShared_reserved: $reserved_shared\n";
      ...
      push @out,
        "Num_blocked: " . scalar(grep {$pe{$_}->{blocked}} keys(%pe)) . "\n";
    }
    if ($flags =~ /p/) {
      push @out,
          "Free_own: "
  ...
  if ($flags =~ /O/) {
    push @out,
      "Max_np: " . get_setting('max_np', $user, '', $cluster_name) . "\n";
    push @out,
      "Max_queue: " . get_setting('max_queue', $user, '', $cluster_name) . "\n";
    push @out,
      "Max_sum_np: "
    ...
    push @out,
      "Max_tasks: " . get_setting('max_tasks', $user, '', $cluster_name) . "\n";
    push @out,
      "Priority: " . get_setting('priority', $user, '', $cluster_name) . "\n";
    push @out,
      "Def_priority: "
      . get_setting('def_priority', $user, '', $cluster_name) . "\n";
    foreach $x (keys(%{$sched_user_info{$cluster_name}->{$user}})) {
      push @out, "$x: " . $sched_user_info{$cluster_name}->{$user}->{$x} . "\n";
    }
  }

  if ($flags =~ /s/) {
    push @out, "Blocked: " . (($mode & MODE_QUEUE_ALLOW) ? '0' : '1') . "\n";
  ...
  if ($flags =~ /o|f/ or keys(%users)) {
    ...
    push @out,
      "Pre-runned: " . scalar(grep({$_->{state} eq 'prerun'} @queue)) . "\n";
    push @out,
      "Pending: " . scalar(grep({$_->{state} eq 'blocked'} @queue)) . "\n";
    ...
    foreach $x (@running, @foreign, @pending, @queue) {
      ...
      push @out,
        "Id: $x->{id}\nState: $x->{state}\nUser: $x->{user}\nNp: $x->{np}\n";
      push @out, "Extracpus: " . scalar(@{$x->{extranodes}}) . "\n";
      push @out,
"Time: $x->{time}\nBlocked: $x->{blocked}\nOwner: $x->{owner}\nOwner_id: $x->{oldid}\n";
...
#  foreach my $p (@foreign,@running) {
  foreach my $p (@running) {
    $$free_total  -= scalar(@{$p->{shared}}) + scalar(@{$p->{own}});
...
"[global]\nfree= $free_total\nshared_free= $free_shared_total\nshared_reserved= $reserved_shared\n";
  ...
  push @out, "blocked_pe= " . join(',', sort(@blocked)) . "\n";
  {
    ...
    foreach $x (sort(keys(%blocked_pe_reasons))) {
      push @out,
        "${delimiter}${x}|" . join(";", sort(keys(%{$blocked_pe_reasons{$x}})));
      $delimiter = ",";
    }
    push @out, "\n";
  }
  ...
  push @out, "foreign= " . (0 + grep($_->{state} eq 'prerun',  @queue)) . "\n";
  push @out, "pending= " . (0 + grep($_->{state} eq 'blocked', @queue)) . "\n";
  push @out, "queue= " .   (0 + grep($_->{state} eq 'queued',  @queue)) . "\n";
  push @out, "mode= $mode\n";
  push @out, "autoblocks= " . join(",", keys(%autoblock)) . ";\n";
  push @out, "autononblocks= " . join(",", keys(%autononblock)) . ";\n";
  push @out, "all_autoblock= $all_autoblock;\n";
  push @out, "norun= " . (($mode & MODE_RUN_ALLOW) ? '0' : '1') . "\n";
  push @out, "noqueue= " . (($mode & MODE_QUEUE_ALLOW) ? '0' : '1') . "\n";
  push @out, "autorestart= " . (($mode & AUTORESTART) ? '1' : '0') . "\n";
  ...
  for $x (@running, @foreign, @pending, @queue) {
    ...
    if (ref($x->{blocks}) ne 'ARRAY') {
      ...
      if (scalar(@{$x->{blocks}}) > 0) {
        push @out, "blockslist.$id = " . join('#', @{$x->{blocks}}) . "\n";
      }
    }
    foreach $tmp (keys(%$x)) {
      $s = $tmp;
    ...
    if ($val) {    # blocking
      ...
      foreach $x (@{$q->{blocks}}) {
        ...
        if (($u eq $user) && ($r eq $reason)) {
          ...
          $comment = "Task $q->{id} is already blocked by $u for '$r'";
    ...
    if ($last_time > $next_time_to_block) {
      ...
      foreach my $i (keys(%to_block)) {
        if ($to_block{$i}->{time} < $last_time) {
          block_pe($i, 1, 0, keys(%{$to_block{$i}->{reason}}));
          delete $to_block{$i};
        } elsif ($next_time_to_block == 0
          || $to_block{$i}->{time} < $next_time_to_block) {
          $next_time_to_block = $to_block{$i}->{time};   # new min time to block
        }
      }
    }
  }
...
"-Invalid interval '$i'! ($result_count processor(s) ${un}blocked).\n";
        ...
        for ($i = $a2; $i <= $b2; ++$i) {
          $result_count +=
            block_one_pe("${a1}${i}${a3}", $val, $when_free, @reasons);
        }
      } else {    # Single node/pe
        qlog "BLOCK_NODE: $i\n", LOG_INFO;
        $result_count += block_one_pe($i, $val, $when_free, @reasons);
      }
    }
    if ($result_count) {
      $q_change = 1;
...
#                            );
      ...
      foreach $pe (@pes) {
        if ($pe{$pe}->{blocked}) {
          ...
          next
            if (scalar(keys(%{$blocked_pe_reasons{$pe}})) > 0);
          ...
          if (exists($to_block{$pe})) {
            ...
            if (scalar(keys(%{$to_block{$pe}->{reason}})) > 0) {
              qlog "Still delayed cause "
                . join(',', keys(%{$to_block{$pe}->{reason}}))
                . "\n", LOG_INFO;
              next;
            }
            delete $to_block{$pe};
    ...
    if ($result_count - $delayed_count > 0) {
      foreach $i (@reasons) {
        my %args = ('pe', $node, 'val', $val, 'safe', $when_free, 'reason', $i);
        qlog
        ...
        &main::new_req_to_child(
          'block_pe',         \%args,
          '__all__',          1,
          SUCC_OK | SUCC_ALL, \&main::chld_block_pe_handler,
...

{
  ...
  sub get_block_x($ ) {
    ...
    my $h = $_[0];
    my ($line, $tmpchar, $incomplete, $error, $success, $tmp, $any, @ret);
...
sub kill_tree ($$) {
  my ($signum, $pid) = @_;
  my (
    %childs,         # list pid's childs (including itself)
...
"sub cleo_log( \$ ){cleosupport::qlog(\"MOD: \$_[0]\",cleovars::LOG_INFO);};\n",
            ...
            "sub launch(\$\$\$){cleosupport::launch(\$_[0],\$_[1],\$_[2])};\n",
            $modules_prolog,
            @content,
            "\n");
          eval $ev;
          ...
          if ($@) {
            ...
            qlog "Loaded exec module $i (version " . ${$v . "::cleo"} . ")\n",
              LOG_INFO;
            $exec_modules{$i} = $v;

            # create defined and undefined module methods
            foreach $j (
              'pre', 'post', 'ok',    'fail', 'cancel', 'attr',
              'add', 'del',  'block', 'init', 'wake'
              ) {
              if ($v->can($j)) {
                qlog "Defined $i/$j()\n", LOG_DEBUG;
            ...
            # do inititialize...
            do_exec_module($i, 'init', undef);
          } else {
            qlog "Missing module version\n", LOG_WARN;
          }
        } else {
...
#
#  Do action in all exec_modules in stack
#
#  Args:
...
#
####################################
sub do_all_exec_modules($$$;@) {
  my ($act, $entry, $user, @args) = @_;
  my $res    = 0;
  my @cancel = ();    # save all exec_mod_cancel values

  my $modlist = get_setting('use_exec_modules', $user, $entry->{profile});
  if (defined(@$modlist)) {
    foreach my $i (@$modlist) {
      $res |= do_exec_module($i, $act, $entry, @args);
      push @cancel, $exec_mod_cancel if ($exec_mod_cancel ne '');
    }

    # restore exec_mod_cancel
    $exec_mod_cancel = join(';', @cancel);
    return $res;
  }
}
...
sub do_exec_module( $$$;@ ) {
  ...
  my $t;
  if (defined $task) {
    $t = Storable::thaw(Storable::freeze($task));
      ...
      foreach $i (@$mlist) {
        ...
        #          qlog join('',"\npackage CleoScheduler::$m;\n",@content,"\n");
          ...
          if ($@) {
            ...
            qlog "Loaded scheduler $i (version " . ${$v . "::cleo"} . ")\n",
              LOG_INFO;
            $ext_sched{$i} = $v;
  ...
  sub sc_execute( ;$ ) {
    ...
    for (my $i = 0; $i <= $#old_sc_tasks; ++$i) {
      if (($old_sc_tasks[$i]->{time} <= $last_time) or $force) {
        eval {$old_sc_tasks[$i]->{sub}->(@{$old_sc_tasks[$i]->{args}});};
      } else {
...
sub do_rotate( $ ) {
  ...
  my @tim  = localtime();
  my $date =
    strftime("%Y-%m-%d", $tim[0], $tim[1], $tim[2], $tim[3], $tim[4], $tim[5]);

  qlog "Rotating '$file'\n", LOG_INFO;

  # tell childrens to reopen logs
  main::new_req_to_child(
    'reopen_logs', {}, '__all__', 1,
  ...
  launch(
    0,
"/bin/sh -c '$cmd < \"${file}.rot\" > \"${file}.$date.$ext\" && rm \"${file}.rot\"'",
    'logrotate'
  );
}
...
sub log_rotate() {
  ...
  ( undef,  undef, undef, undef,  undef, undef, undef,
    $lsize, undef, undef, $ltime, undef, undef
  ) = lstat($report_file);
  ( undef,  undef, undef, undef,  undef, undef, undef,
    $ssize, undef, undef, $stime, undef, undef
  ) = lstat($short_rep_file);
      ...
      if ($ltime + $time * 24 * 3600 < $last_time) {
        if (rename($report_file, "$report_file.rot")) {
          ...
          unless (
            ...
            send_mail(get_setting('adm_email'),
              'cleo error', 'Cannot recreate log file');
...
sub violates( $;$ ) {
  if (exists($ids{$_[0]})) {
    ...
    # is blocked?
    return 1
      if ((ref($q_entry->{blocks}) eq 'ARRAY')
    ...
    if ($tmp > -1 and ($q_entry->{timelimit} * $np_real) > $tmp * 3600) {
      if ($_[1]) {
        block_task($q_entry->{id}, 1, '__internal__',
          'maximum cpu*hours reached');
      }
      return 1;
    }
    ...
    if (main::can_run($q_entry)) {
      if ($q_entry->{attrs}->{aftertime} > $last_time) {
        if ($_[1]) {
          block_task($q_entry->{id}, 1, '__internal__',
            'aftertime attribute restriction');
        }
        return 1;
      }
...
sub reopen_logs() {
  ...
  unless ($cleosupport::SHORT_LOG->open(
      $short_rep_file, O_WRONLY | O_CREAT | O_APPEND | O_LARGEFILE)
    ) {
  ...
  sub flush_mails(;$) {
    ...
    foreach $addr (keys(%mails)) {
      ...
      foreach $subj (keys(%{$mails{$addr}})) {
        ...
        # is it time to send?
        if ( $last_time > $mails_lt{$addr}->{$subj} + $delay
          or $last_time > $mails_ft{$addr}->{$subj} + $max_delay
          or $_[0] > 0) {
...
#
#  Set wakeup timer for exec_module or scheduler
#  Args:
#        package  - module/scheduler name
...
sub set_mod_timer( $;$$$$$$ ) {
  ...
  if ($add) {
    ...
    # adding new timer
    push @{$mod_timers{$pack}},
      [$min, $hour, $wday, $mday, $mon, $add, $data, 0];
  } else {
    ...
    foreach my $t (@{$mod_timers{$pack}}) {
      ...
      # test mask
      if (  ($t->[0] == $min or $min == '')
        and ($t->[1] == $hour or $hour == '')
        and ($t->[2] == $wday or $wday == '')
        and ($t->[3] == $mday or $mday == '')
        and ($t->[4] == $mon  or $mon == '')
        and ($t->[5] == $data or $data == '')) {
...
#
#  Check module timers for action...
#
#  Args:
#        module - module name
#
...
sub check_mod_timer($) {
  ...
  my ($min, $hour, $mday, $month, $wday, $next, $min_next);
  my ($i, $t, $set);

  (undef, $min, $hour, $mday, $month, undef, $wday) = localtime($last_time);

  # t: [0=min, 1=hour, 2=wday, 3=mday, 4=mon, 5=data, 6=last_run_time ]
  # check all timers...
  for ($i = 0; $i <= $#{$mod_timers{$mod}}; ++$i) {
    $t = $mod_timers{$mod}->[$i];

    # check if we did not run it last hour already
    next if ($last_time - $t->[6] < 3600);
    ...
    # All matches!!!
    # Call module
    qlog "Do module timer ($mod)\n", LOG_DEBUG;
    eval {
      local $SIG{ALRM} = sub {die "module_timer '$mod' timed out\n";};
      no strict "refs";
      alarm get_setting('exec_modules_timeout');
      my $m   = $exec_modules{$mod};
      my $res = ($m . "::wake")->($t->[5]);
...
#
#  Check all active modules for timer action...
#
#
##############################################################
