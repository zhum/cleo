#!/usr/bin/perl
#
#  Cleo visualisation tool
#  This is a cgi-script, which visualizes current Cleo state in several modes
#
...
# SKIF Chebyshev example:
#my @all_queues=('regular','hdd','hddmem','bigmem','test');


my $def_mode='table';  #default mode
                       # table = nodes table
                       # tasks = tasks list
                       # image = One big picture
                       # index = short resume

# Change, if you unstall it in another place
my $myname='/cgi-bin/cleo-viz.cgi';


# Usually you don't want to change theese vars
#my $tmpl_prefix='/var/lib/cleo-viz';
my $tmpl_prefix='__LIB__';
...
if(($mode eq 'html') or ($force_html==1)){
    ...
    foreach my $q (@queues){
        ...
        print "<tr><td class='qname' colspan='7'><div id='queue' run='$tasks_run{$q}' all='$tasks_all{$q}'\
              ...
              $q ($trans->{total}: $total_cpus{$q}, $trans->{free_p}:\
              $free_total{$q}, $trans->{block}: $blocked_cpus{$q})</div></td></tr>\n";
...
<!--
function print_wait(){
\$("#info").html("<center>$trans->{wait} <img src='$wait_img'></center>");
};
...
\$(document).ready(function(){
  ...
  do_request=function(){
    ...
    \$.ajax({
      ...
      success:    function(answer){ \$("#info").html(answer); reload_users(); trigger_tasks_table();},
...
<form onSubmit="return false">
  ...
  <option value=table selected>$trans->{nodeslist}</option>
  <option value=tasks>$trans->{taskslist}</option>
  <option value=image>$trans->{oneshot}</option>
